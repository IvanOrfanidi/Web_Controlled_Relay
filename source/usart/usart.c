...
#if defined(__AVR_ATmega48__) || (__AVR_ATmega48P__) || (__AVR_ATmega88__) || \
    (__AVR_ATmega88P__) || (__AVR_ATmega168__) || \
    (__AVR_ATmega168P__) //If ATmega 48/64/168
...
void usart0_init()
{
	...
	uint8_t status = (0 << UMSEL01) | (0 << UMSEL00);
...
{
	if(tx_counter_index0 == TX_BUFFER_SIZE0) {
		tx_counter_index0 = 0;
	}
...
ISR(USART_UDRE_vect)
{
	if(tx_wr_counter_index0 == TX_BUFFER_SIZE0) {
...
ISR(USART_RX_vect)
{
	if(!(UCSR0A & ((1 << DOR0) | (1 << FE0) | (1 << UPE0)))) {
		if(rx_counter_index0 == RX_BUFFER_SIZE0) {
			rx_counter_index0 = 0;
		}
...
uint8_t usart0_read()
{
	if(rx_rd_counter_index0 == RX_BUFFER_SIZE0) {
...
{
	return rx_counter_index0 - rx_rd_counter_index0;
}
...
{
	if(tx_counter_index1 == TX_BUFFER_SIZE1) {
		tx_counter_index1 = 0;
	}
...
{
	if(tx_wr_counter_index1 == TX_BUFFER_SIZE1) {
...
ISR(USART1_RXC_vect)
{
	if(!(UCSR1A & ((1 << DOR0) | (1 << FE0) | (1 << UPE0)))) {
		if(rx_counter_index1 == RX_BUFFER_SIZE1) {
			rx_counter_index1 = 0;
		}
...
uint8_t usart1_read()
{
	if(rx_rd_counter_index1 == RX_BUFFER_SIZE1) {
...
{
	return rx_counter_index1 - rx_rd_counter_index1;
}
