...
void Init_ip_arp_udp_tcp(const uint8_t* mymac,
                         const uint8_t* myip,
                         uint16_t wwwp)
{
	uint8_t i = 0;
...
void Make_udp_reply_from_request(uint8_t* buf,
                                 char* data,
                                 uint8_t datalen,
                                 uint16_t port)
{
...
uint16_t Fill_tcp_data_len(uint8_t* buf,
                           uint16_t pos,
                           const char* s,
                           uint8_t len)
{
...
void Make_tcp_ack_with_full_data(uint8_t* buf,
                                 uint16_t dlen) // Flag Fin disable.
{
	uint16_t j;
	...
	// the fin here:
	buf[TCP_FLAG_P] = TCP_FLAG_ACK_V |
	                  TCP_FLAG_PUSH_V; //| TCP_FLAG_FIN_V; // Flag Fin disable.
